...
class Postfilter:
    ...
    def get_filtered(self, dedup_annot):
        ...
        """
        quality_annotations = list(filter(lambda an: an.has_required_modules(self.postfilter_repetitions) and
        ...
        filtered_annotations = list(filter(lambda an: an not in quality_annotations, dedup_annot))
        filt_primer = list(filter(lambda an: an.has_one_primer(self.postfilter_bases, self.postfilter_repetitions, self.index_rep - 1, None if self.index_rep2 is None else self.index_rep2 - 1) and
                                        an.has_less_errors(self.postfilter_errors, self.postfilter['max_errors_relative']), filtered_annotations))
        filtered_annotations = list(filter(lambda an: an not in quality_annotations and an not in filt_primer, dedup_annot))
